...
class conv_net():
    ...
    def evaluate_timeseries(timeseries, window_size, valid_split=0.15, test_split=0.3):
        ...
        model = self.make_1d_convnet(window_size=window_size, filter_length=filter_length, nb_input_series=nb_series,
    ...
    #np.atleast_3d(np.array([timeseries[start:start + window_size] for start in range(0, timeseries.shape[0] - window_size)]))